...
export const PortfolioCarousel: React.FC = () => {
  return (
    <section className="py-20 bg-slate-950 relative overflow-hidden border-t border-white/5">
      {/* Background Decor */}
      <div className="absolute inset-0 bg-[linear-gradient(to_bottom,transparent,rgba(6,182,212,0.05),transparent)] pointer-events-none" />
      ...
      <div className="relative w-full flex overflow-hidden">
        ...
        >
          ...
          {[...projects, ...projects, ...projects].map((project, index) => {
            const CardContent = (
              <div className="bg-slate-900 border border-white/10 rounded-xl overflow-hidden shadow-xl transition-transform duration-300 group-hover:-translate-y-2 group-hover:shadow-[0_10px_40px_rgba(6,182,212,0.15)] group-hover:border-cyan-500/30 h-full">
                ...
                <div className={`h-48 md:h-56 bg-gradient-to-br ${project.gradient} relative flex items-center justify-center overflow-hidden`}>
                   ...
                   <h3 className="text-3xl font-black text-white/20 uppercase tracking-widest transform -rotate-12 group-hover:scale-110 transition-transform duration-500">
                     {project.title}
                   </h3>
                </div>
            ...
            return (
              <div
                key={index}
                className="relative group w-[300px] md:w-[400px] flex-shrink-0 cursor-pointer"
              >
                {project.link !== "#" ? (
